...
/datum/species/regenerate_organs(mob/living/carbon/target, datum/species/old_species, replace_current = TRUE, list/excluded_zones, visual_only = FALSE, replace_missing = TRUE)
	...
	if(target.dna.features["frame_list"] && !(type in GLOB.species_blacklist_no_humanoid))
		//head
		if(target.dna.features["frame_list"][BODY_ZONE_HEAD] && (type == /datum/species/android || type == /datum/species/android/holosynth))
...
/datum/preference/choiced/head_type/is_accessible(datum/preferences/preferences)
	...
	if(species == /datum/species/android || species == /datum/species/android/holosynth) // lifting this restriction would require code for the head's internal organs to become cybernetic too
		return TRUE
	return FALSE
